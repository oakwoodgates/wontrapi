...
class Wontrapi_Extension_Demo {
	...
	/**
	 * Init the addon. Stash and return the object.
	 * 
	 * @return object
	 * @since  0.1.0
	 */
		...
		if ( ! isset( $wontrapi_xd_fs ) ) {
			...
			if ( file_exists( dirname( dirname( __FILE__ ) ) . '/wontrapi/vendor/freemius/start.php' ) ) {
				...
				$wontrapi_xd_fs = fs_dynamic_init( array(
					...
					'premium_slug'        => 'wontrapi-extension-demo',
					'type'                => 'plugin',
					...
					'secret_key'          => 'sk_oEK!?3IvVCZ-!d~hDcSYHo-g._%<)',
				) );
			}
		}
...
function wontrapi_xd_fs_is_parent_active() {
	...
	foreach ( $active_plugins as $basename ) {
		if ( 0 === strpos( $basename, 'wontrapi/' ) ||
			0 === strpos( $basename, 'wontrapi-premium/' )
		) {
			return true;
		}
	}
...
// Kick it off.
add_action( 'plugins_loaded', array( wontrapi_xd(), 'hooks' ), 10 );
